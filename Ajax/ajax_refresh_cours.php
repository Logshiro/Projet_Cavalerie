    ...
    if ($req->execute()) {
        ...
        if (!empty($list)) {
            foreach ($list as $res) {
                ...
                echo '<li onclick="set_item(\'' . str_replace("'", "\'", $res['LibGalop']) . '\', ' . $res['idGalop'] . ')">' . $Listecategorie . '</li>';
            }
        } else {
    ...
    if ($req->execute()) {
        ...
        if (!empty($list)) {
            foreach ($list as $res) {
                ...
                echo '<li onclick="set_item_Insert(\''. str_replace("'", "\\'", htmlspecialchars($res['LibGalop'], ENT_QUOTES)) .'\', ' . htmlspecialchars($res['idGalop']) . ')">' . $Listecategorie . '</li>';
            }
        } else {
            echo 'Aucun résultat trouvé';
        }
